...
print.names.under.taxon(my.list = limony, taxon = "Microcystaceae")

# And specifically like it is "Microcystis.Microcystaceae"
# Now I can use this spelling to subset out Microcystis in August:
flat.lim <- flatten.to.single.level(my.list = limony)
index <- get.taxon.indexes(my.list = flat.lim, taxa = "Microcystis.Microcystaceae")
microcystis <- subset.by.taxa(my.list = flat.lim, keep.index = index)
index <- get.sample.indexes(my.list = microcystis, start.YY.MM.DD = "00-8-1", end.YY.MM.DD = "19-8-31", dates.are.season.range = TRUE)
